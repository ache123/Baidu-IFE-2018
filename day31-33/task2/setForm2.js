...
//获取数据
function getData() {
    let regionName = getName(region.value, regionList);
    let productName = getName(product.value, productList);
    let tempArr = [];

    for (let i of sourceData) {
        console.log(i);
        if (i.region == regionName && i.product == productName) {
            tempArr.push(i);
        }
    }
